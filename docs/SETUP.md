...
除了 Google Gemini，本项目还支持 DeepSeek、智谱 (ZhiPu) 和腾讯混元 (Tencent Hunyuan)。
...
1.  **获取 API Key**:
    ...
    *   **腾讯混元**: 访问 [腾讯混元控制台](https://cloud.tencent.com/product/hunyuan) 获取 API Key。

2.  **配置环境变量**:
    在 `.env` 文件中，添加相应模型的 API Key（不需要的供应商可以不配）：
    ```env
    # ... 其他配置
...
3.  **指定默认模型 (可选)**:
    你可以通过 `MODEL` 环境变量设置启动时默认使用的模型。值为模型注册名：
    ```env
    # ... 其他配置
    MODEL=hunyuan-turbos-latest
    ```
    可选值：`gemini-3-flash-preview`、`deepseek-v3.2`、`glm-5`、`hunyuan-turbos-latest`。
    如果不设置，程序将使用第一个成功注册的模型。
...
```text
[DEBUG] Using preferred model: hunyuan-turbos-latest
Current Model: Hunyuan-Turbos
AI Chat (type '/exit' to quit, '/new' to start a new chat, '/use <model>' to switch, '/agents' to list agents, '/skills' to list skills)
...
你可以在聊天中随时使用 `/use` 命令切换模型，参数为模型注册名。
