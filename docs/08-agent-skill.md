...
SkillRegistry 支持两种文件布局，从 `.agent/skills/` 目录加载：

```
.agent/skills/
├── my-skill/            # 目录布局（推荐，符合 Agent Skills 标准）
│   └── SKILL.md
├── another-skill/
│   └── SKILL.md
└── quick-fix.md         # 扁平布局（兼容）
```

- **目录布局**（推荐）：每个子目录包含一个 `SKILL.md` 文件，目录名作为默认 name
- **扁平布局**（兼容）：直接放置 `.md` 文件，文件名（去掉 `.md`）作为默认 name
