...
async send(text: string): Promise<string> {
  ...
  for (let i = 0; i < 10; i++) {       // 安全阀：最多 10 圈
...
`maxRounds = 10` 是安全阀，防止 AI 死循环。

## 扩展

当前只有一个工具（执行终端命令），但 `ToolRegistry` 支持随时扩展：

```typescript
registry.register(new RunShellCommand());     // 执行命令
// registry.register(new ReadFile());          // 读文件
// registry.register(new WriteFile());         // 写文件
// registry.register(new WebSearch());         // 搜索网页
```
